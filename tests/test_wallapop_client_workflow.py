...
class TestWallapopClientLoginWorkflow(unittest.TestCase):
    ...
    def test_login_workflow_sequence(self):
        ...
        with patch.object(self.client.session_manager, 'load_session', return_value=False), \
             patch.object(self.client, '_fresh_login', return_value=False) as mock_fresh, \
             patch.object(self.client, '_manual_cookie_login', return_value=True) as mock_manual, \
    ...
    def test_login_uses_existing_session_when_valid(self):
        ...
        with patch.object(self.client.session_manager, 'load_session', return_value=True), \
    ...
    def test_manual_fallback_defaults_to_yes(self):
        ...
        with patch.object(self.client.session_manager, 'load_session', return_value=False), \
             ...
             patch.object(self.client, '_manual_cookie_login', return_value=True) as mock_manual, \
             patch('builtins.input', return_value=''):  # Empty string (Enter key)
    ...
    def test_fingerprint_randomization(self, mock_random_choice):
        ...
        with patch('wallapop_auto_adjust.wallapop_client.SessionManager'):
            ...
            self.assertEqual(fingerprint['viewport'], [1920, 1080])  # JSON stores as list
    ...
    def test_fingerprint_persistence(self):
        ...
        with patch('wallapop_auto_adjust.wallapop_client.SessionManager'):
            ...
            with patch('pathlib.Path.exists', return_value=True), \
                ...
                self.assertEqual(fingerprint['viewport'], [1920, 1080])  # JSON returns list
