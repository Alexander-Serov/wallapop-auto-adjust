...
class FakeResponse:
    def __init__(self, status_code: int = 200, data: Optional[dict] = None, set_cookie: Optional[str] = None):
        ...
        if set_cookie:
            ...
            try:
                ...
                if set_cookie.startswith('accessToken='):
                    ...
                    self.cookies.set('accessToken', value, domain='.wallapop.com', path='/')
            except Exception:
                pass
...
def seed_required_cookies(jar: RequestsCookieJar):
    jar.set('__Secure-next-auth.session-token', 'S'*1200, domain='es.wallapop.com', path='/', secure=True)
    jar.set('__Host-next-auth.csrf-token', 'C'*64, domain='es.wallapop.com', path='/', secure=True)
    jar.set('__Secure-next-auth.callback-url', 'https%3A%2F%2Fes.wallapop.com%2Fapp%2Fchat', domain='es.wallapop.com', path='/', secure=True)
...
def test_refresh_token_from_set_cookie_header(monkeypatch):
    def responder(url: str, kwargs: Dict):
        if url.endswith('/api/auth/federated-session'):
            return FakeResponse(200, data={}, set_cookie='accessToken=COOKIE_TKN; Path=/; Domain=.wallapop.com')
        return FakeResponse(200, data={})
...
def test_query_param_fallback_uses_session_token(monkeypatch):
    ...
    assert any('/api/auth/federated-session' in url and ('params' in kw and 'token' in (kw.get('params') or {})) for _, url, kw in spm.session.calls)
...
def test_nudge_users_me_sets_cookie(monkeypatch):
    def responder(url: str, kwargs: Dict):
        ...
        if '/api/v3/users/me' in url:
            ...
            return FakeResponse(200, data={}, set_cookie='accessToken=NUDGE_TKN; Path=/; Domain=.wallapop.com')
        return FakeResponse(200, data={})
...
def test_warmup_calls_precede_federated_session():
    ...
    first_fed_index = next(i for i, (_, url, _) in enumerate(spm.session.calls) if url.endswith('/api/auth/federated-session'))
...
def test_make_authenticated_request_sets_bearer_header():
    ...
    resp = spm.make_authenticated_request('GET', 'https://api.wallapop.com/api/v3/users/me/')
    assert resp.status_code == 200
    # Find the API call and inspect headers
    api_calls = [(m, url, kw) for (m, url, kw) in spm.session.calls if url.startswith('https://api.wallapop.com/api/v3/users/me/')]
...
def test_make_authenticated_request_retries_on_401():
    ...
    user_calls = [c for c in spm.session.calls if c[1].startswith('https://api.wallapop.com/api/v3/users/me/')]
